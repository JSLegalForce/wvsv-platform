...
const WvSvUpload = (() => {
  ...
  function normalizePdfText(raw){var t=raw;t=t.replace(/[\uFFFD\u25AA\u25A0\u2022\u00B7\u25CF]/g," ");t=t.replace(new RegExp("(Artikel[\t ]+[0-9]+(?:[.][0-9]+)+)","g")," $1");t=t.replace(/ {3,}/g,"  ");return t.trim();} function verwerkBestand(bestand){
  ...
  function onAnalyseer(){
    ...
    leesBestand(state.currentFile).then(function(tekst){
      ...
      var ext2=state.currentFile.name.split('.').pop().toLowerCase();if(ext2==='pdf'){tekst=normalizePdfText(tekst);logDebug('Na normalisatie: '+tekst.length+' tekens');}var artikelen=WvSvParser.parseerArtikelen(tekst);
