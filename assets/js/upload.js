/** upload.js v12 - structuurboom + artikelweergave */
const WvSvUpload = (() => {
  const state = {currentFile:null,currentSource:null,parsedArticles:[],loadedItems:[],jsonData:null,filterQuery:'',currentPage:1,pageSize:50,activeBoek:null,structuurData:null,activeTitelNr:null};
  ...
  function resetState(){state.currentFile=null;state.currentSource=null;state.parsedArticles=[];state.loadedItems=[];state.jsonData=null;state.filterQuery='';state.currentPage=1;state.activeBoek=null;state.structuurData=null;state.activeTitelNr=null;}
  function resetUI(){
    ['upload-teller','upload-paginering','upload-stats','upload-structuur'].forEach(function(id){var el=$id(id);if(el)el.innerHTML='';});
  ...
  function toonStructuurBoom(structuur) {
    ...
    if (!structuur || !structuur.boeken || !structuur.boeken.length) {
      el.innerHTML = '<div style="color:#888;font-size:0.85rem;">Geen structuurkoppen gevonden.</div>';
      return;
    }
    ...
    var html = '<div style="font-weight:700;font-size:0.85rem;margin-bottom:10px;color:var(--blauw);">'
      + 'STRUCTUUR: ' + structuur.boeken.length + ' boeken · ' + totHfst + ' hoofdstukken · ' + totTitel + ' titels'
      + '</div>';
    html += '<div style="font-family:sans-serif;font-size:0.82rem;line-height:1.8;">';
    structuur.boeken.forEach(function(boek) {
      html += '<div style="margin-bottom:6px;">';
      html += '<div style="font-weight:700;color:#fff;background:var(--blauw);padding:4px 10px;border-radius:5px;display:inline-block;cursor:pointer;margin-bottom:4px;" onclick="WvSvUpload.toggleBoom(this)">';
      html += '&#9660; Boek ' + e(boek.nummer) + ': ' + e(boek.titel);
      html += ' <span style="opacity:0.7;font-weight:400;">(' + boek.hoofdstukken.length + ' hfst.)</span></div>';
      ...
      boek.hoofdstukken.forEach(function(hfst) {
        ...
        html += '<div style="font-weight:600;cursor:pointer;padding:2px 4px;" onclick="WvSvUpload.toggleBoom(this)">';
        html += '&#9660; Hfst. ' + e(hfst.nummer) + ' — ' + e(hfst.titel);
        html += ' <span style="color:#888;font-weight:400;">(' + hfst.titels.length + ' titels)</span></div>';
        ...
        hfst.titels.forEach(function(titel) {
          ...
          html += '<div style="cursor:pointer;padding:1px 4px;color:var(--blauw);text-decoration:underline;font-size:0.8rem;" onclick="WvSvUpload.laadTitel(\'' + e(titel.nummer) + '\')">';
          html += '&#9654; Titel ' + e(titel.nummer) + ' — ' + e(titel.titel);
          html += ' <span style="color:#888;text-decoration:none;">(' + nrArt + ' art.)</span></div>';
        });
        html += '</div></div>';
      });
      html += '</div></div>';
  ...
  function gefilterd(){
    ...
    if(state.activeTitelNr){lijst=lijst.filter(function(a){var d=a.artikel.split('.');return d.length>=3&&d[0]+'.'+d[1]===state.activeTitelNr;});}
    else if(state.activeBoek){lijst=lijst.filter(function(a){return(a.artikel.split('.')[0])===state.activeBoek;});}
  ...
  function toonPagina(){
    ...
    if(teller)teller.textContent=label?(totaal+' van '+state.parsedArticles.length+' artikelen ('+label+')'):state.parsedArticles.length+' artikel'+(state.parsedArticles.length!==1?'en':'' )+' gevonden';
  ...
  function filterBoek(boek){state.activeBoek=state.activeBoek===boek?null:boek;state.activeTitelNr=null;state.currentPage=1;toonStats(state.parsedArticles);toonPagina();}
  function toonResultaten(artikelen){
    state.parsedArticles=artikelen;state.jsonData=JSON.stringify(artikelen,null,2);state.currentPage=1;state.filterQuery='';state.activeBoek=null;state.activeTitelNr=null;
    var z=$id('upload-zoek');if(z)z.value='';
  ...
  function onAnalyseer(){
    ...
    leesBestand(state.currentFile).then(function(tekst){
      ...
      if (totBoeken > 0) { toonStructuurBoom(structuur); }
      else { logDebug('Geen structuurkoppen — val terug op artikelparsing'); }
  ...
  function onZoek(ev){state.filterQuery=ev.target.value;state.currentPage=1;state.activeTitelNr=null;toonPagina();}
  ...
  function init(){
    ...
    logDebug('v12 klaar — PDF.js: '+(typeof pdfjsLib!=='undefined'?'OK':'ONTBREEKT')+' | Mammoth: '+(typeof mammoth!=='undefined'?'OK':'ONTBREEKT'));
  }
